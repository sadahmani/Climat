...
def load_obs_data(obs_filename=None, data_dir=None, obs_label='OBS',
    ...
    try:
        ...
        if obs_label == 'OBS':
            ...
            if obs_filename is None :
                obs_filename = 'HadCRUT/HadCRUT.5.0.1.0.analysis.summary_series.global.annual.nc'
            ...
            if obs_filename is None :
                obs_filename = 'HadCRUT/HadCRUT.5.0.1.0.analysis.ensemble_series.global.monthly.nc'
            if nc_varname is None :
                nc_varname = 'tas'
            if preindustrial_mean_shift_flg is None:
                hadcrut_preindustrial_mean_shift_flg = True
...
def associated_test_label(data_and_training_label) :
    ...
    splited_dgen_Npc_particle = dgen_Npc_particle.split('_')
    if splited_dgen_Npc_particle[0] == 'N132' :
        test_N_part = '_'.join('NMx100', *splited_dgen_Npc_particle[1:])
        
    elif splited_dgen_Npc_particle[0] == 'N1000' :
        test_N_part = '_'.join('NMx1000', *splited_dgen_Npc_particle[1:])
...
def retrieve_param_from_base_case(base_case_label, verbose=False) :
    ...
    bctoex_dgenNandZone_particle = splited_base_case[4]
    ...
    nandz_splited = bctoex_dgenNandZone_particle.split('_')
    if nandz_splited[1][0] not in ['v','z']:
        raise NameError("nandz_splited[1][0] not in ['v','z']")
    if nandz_splited[1][0] == 'z' :
        region = nandz_splited[1][1:3]
    else:
        region = None 
    print('REGION:',region)
    ...
    try :
        ...
        if 'z{}v0'.format(region) not in splitedunder_for_data_label :
            raise NameError(f"\n  ** unspected information in part of base directory name. Expected to find the 'v4' particule,\n  ** in the '{joined_for_data_label}' part of base directory. Is a correct case?")
