        ...
        for innet in np.arange(n_nnets):
            ...
            if not force_plot and save_figs and os.path.isfile(figs_filename):
                ...
                for iax,inbe in enumerate(ilist_of_nbe) :
                    Yinv_arr,nbe = mlt_list_Yinv[inbe],mlt_list_nbe[inbe]
    
                    ax = axes.flatten()[iax]  #iax//ncols,iax%ncols]
                    tmp_arr = Yinv_arr.squeeze(axis=1)
...
def plot_averaged_inverted_profiles_by_net (base_case_to_explore, sub_case_to_explore, inversion_suffix, settings_label=None,
    ...
    for i_mod,mod_to_invert in enumerate(models_to_plot) : 
        ...
        for innet in np.arange(n_nnets):
            ...
            mlt_list_mean_Xinv = np.array([Xinv.mean(axis=0) for Xinv in mlt_list_Xinv])
            mlt_list_mean_Yinv = np.array([Yinv.mean(axis=0) for Yinv in mlt_list_Yinv])

            #print(f"#DBG2# mlt_list_mean_Xinv.shape,mlt_list_mean_Yinv.shape",mlt_list_mean_Xinv.shape,mlt_list_mean_Yinv.shape)

            list_for_average_of_mean_Xinv.append(mlt_list_mean_Xinv)
            list_for_average_of_mean_Yinv.append(mlt_list_mean_Yinv)
        ...
        if not force_plot and save_figs and os.path.isfile(figs_filename):
            ...
            for iax,inbe in enumerate(ilist_of_nbe) :
                Yinv_arr,nbe = mlt_for_average_of_mean_Yinv[:,inbe],mlt_list_nbe[inbe]
                print(f"#DBG2# Yinv_arr.shape,nbe",Yinv_arr.shape,nbe)

                ax = axes.flatten()[iax]  #iax//ncols,iax%ncols]
                tmp_arr = Yinv_arr.squeeze(axis=1)   # squeeze ciblé sur l'axe 1
                if False:
                    for ii,y_inv in enumerate(tmp_arr) :
...
def plot_averaged_inv_all_forcings_by_model (base_case_to_explore, sub_case_to_explore, inversion_suffix, settings_label=None,
    ...
    for i_mod,mod_to_invert in enumerate(models_to_plot) : 
        ...
        for innet in np.arange(n_nnets):
            ...
            mlt_list_mean_Xinv = np.array([Xinv.mean(axis=0) for Xinv in mlt_list_Xinv])
            mlt_list_mean_Yinv = np.array([Yinv.mean(axis=0) for Yinv in mlt_list_Yinv])
            mlt_list_std_Xinv = np.array([Xinv.std(axis=0,ddof=1) for Xinv in mlt_list_Xinv])
            mlt_list_std_Yinv = np.array([Yinv.std(axis=0,ddof=1) for Yinv in mlt_list_Yinv])

            list_for_average_of_mean_Xinv.append(mlt_list_mean_Xinv)
            list_for_average_of_mean_Yinv.append(mlt_list_mean_Yinv)
            list_for_average_of_std_Xinv.append(mlt_list_std_Xinv)
            list_for_average_of_std_Yinv.append(mlt_list_std_Yinv)
        ...
        if not force_plot and save_figs and os.path.isfile(figs_filename):
            ...
            for innet in np.arange(n_nnets) :
                ...
                if added_mean_of_all:
                    Xinv_arr = mlt_for_average_of_mean_Xinv[innet,0:-1]
                    Yinv_arr = mlt_for_average_of_mean_Yinv[innet,0:-1]
                    Xinv_arr_std = mlt_for_average_of_std_Xinv[innet,0:-1]
                    Yinv_arr_std = mlt_for_average_of_std_Yinv[innet,0:-1]
                else:
                    Xinv_arr = mlt_for_average_of_mean_Xinv[innet]
                    Yinv_arr = mlt_for_average_of_mean_Yinv[innet]
                    Xinv_arr_std = mlt_for_average_of_std_Xinv[innet]
                    Yinv_arr_std = mlt_for_average_of_std_Yinv[innet]
                ...
                print(f'Xinv_arr_std/Yinv_arr_std shape: {Xinv_arr_std.shape} / {Yinv_arr_std.shape}')
                tmp_xinv_arr = Xinv_arr.squeeze(axis=1)
                tmp_yinv_arr = Yinv_arr.squeeze(axis=1)
                tmp_xinv_arr_std = Xinv_arr_std.squeeze(axis=1)
                tmp_yinv_arr_std = Yinv_arr_std.squeeze(axis=1)
                print(f'tmp_xinv_arr/tmp_yinv_arr shape: {tmp_xinv_arr.shape} / {tmp_yinv_arr.shape}')
                print(f'tmp_xinv_arr_std/tmp_yinv_arr_std shape: {tmp_xinv_arr_std.shape} / {tmp_yinv_arr_std.shape}')
                # plotting inversed X: GHG, AER and NAT

                for x_inv in tmp_xinv_arr :
                    #print('x_inv shape:',x_inv.shape)
                ...
                for iforc,(forc,cs) in enumerate(zip(forcing_names[:3],forcing_colors[:3])) :
                    if forc == 'ghg':
                        FORC_inv_array = GHG_mod_for_inv_array
                    elif forc == 'aer':
                        FORC_inv_array = AER_mod_for_inv_array
                    elif forc == 'nat':
                        FORC_inv_array = NAT_mod_for_inv_array
                    else:
                        FORC_inv_array = None
                    ...
                    current_std_FORC_m = FORC_inv_array.std(axis=0,ddof=1)
                ...
                tmp_xarr_mean = tmp_xinv_arr.mean(axis=0)
                tmp_xarr_std = tmp_xinv_arr.std(axis=0,ddof=1)
                ...
                tmp_y = tmp_yinv_arr.mean(axis=0)
                #print("#DBG3# PRE STD tmp_yinv_arr.std(axis=0,ddof=1) ... tmp_arr.shape",tmp_yinv_arr.shape)
                tmp_y_err = tmp_yinv_arr.std(axis=0,ddof=1)
...
def plot_averaged_inv_all_forcings_by_net (base_case_to_explore, sub_case_to_explore, inversion_suffix,
                                           ...
                                           hatch_for_shaded='//', hatch_for_shaded_inv='\\\\\\', errorlimits_percent=None, errorlimits_n_rms=1,
                                           alpha_for_mean_shaded=0.4, lw_for_mean_shaded=0.5, hatch_for_mean_shaded_inv='\\\\',
                                           force_plot=False, force_write=False, plot_mean_err=True, mean_err_lightness_factor=0.25,
                                           inv_label=None, t_limits=None,
                                           train_years=np.arange(1900,2015),
                                           local_nb_label="PlotAveInvAllForcfByNet", fig_ext='png',
                                           figs_defaults={'dpi':300, 'facecolor':'w', 'edgecolor':'w'},
        ...
        if case_figs_dir is not None and not force_plot and save_figs and os.path.isfile(figs_filename):
            ...
            for i_mod,(mod_to_invert,mod_data_source,mod_sdir) in enumerate(zip(inverted_model_to_plot,source_model_data_to_plot,models_sdirs_to_plot)) : 
                ...
                for i_trmod,mod_from_training in enumerate(models_from_training) :
                    ...
                    mlt_list_std_Xinv = np.array([Xinv.std(axis=0,ddof=1) for Xinv in mlt_list_Xinv])
                    mlt_list_std_Yinv = np.array([Yinv.std(axis=0,ddof=1) for Yinv in mlt_list_Yinv])
                    print(f'mlt_list_mean_Xinv/mlt_list_mean_Yinv shape: {mlt_list_mean_Xinv.shape} / {mlt_list_mean_Yinv.shape}')
                    print(f'mlt_list_std_Xinv/mlt_list_std_Yinv shape:   {mlt_list_std_Xinv.shape} / {mlt_list_std_Yinv.shape}')
                    ...
                    if added_mean_of_all:
                        Xinv_arr = mlt_list_mean_Xinv[0:-1]
                        Yinv_arr = mlt_list_mean_Yinv[0:-1]
                        Xinv_arr_std = mlt_list_std_Xinv[0:-1]
                        Yinv_arr_std = mlt_list_std_Yinv[0:-1]
                    else:
                        Xinv_arr = mlt_list_mean_Xinv
                        Yinv_arr = mlt_list_mean_Yinv
                        Xinv_arr_std = mlt_list_std_Xinv
                        Yinv_arr_std = mlt_list_std_Yinv
                    print(f'Xinv_arr/Yinv_arr shape: {Xinv_arr.shape} / {Yinv_arr.shape}')
                    print(f'Xinv_arr_std/Yinv_arr_std shape: {Xinv_arr_std.shape} / {Yinv_arr_std.shape}')
                    ...
                    # Plotting the means of source data forcings X: GHG, AER and NAT
                    #for iforc,(forc,cs) in enumerate(zip(forcing_names[:3],forcing_colors[:3])) :
                    for iforc,(forc,cs) in enumerate(zip(forcing_names,forcing_colors)) :
                        if forc == 'ghg':
                            FORC_inv_array = GHG_mod_for_inv_array
                        elif forc == 'aer':
                            FORC_inv_array = AER_mod_for_inv_array
                            ...
                            FORC_inv_array = HIST_mod_for_inv_array
                        else:
                            FORC_inv_array = None
                        #current_color_brighter = [[np.min((1,c*1.4)) for c in gt.hexcolor(hexcol)] for hexcol in cycle_color]  # for VAL
                        #c_darker = [ch*0.7 for ch in gt.hexcolor(c)]
                        ...
                        if plot_forc_shaded_region :
                            ...
                            ax.fill(np.NaN, np.NaN, 
                                    ec=cs, fc=gt.lighter_color(cs), alpha=alpha_for_shaded, linewidth=lw_for_shaded,
                                    hatch=hatch_for_shaded, label=f"src {forc.upper()}")
                    ...
                    # -------------------------------------------------------------
                    # Plotting the means of Inverted forcings X: GHG, AER and NAT
                    # if tmp_xinv_arr.shape[0] == 1 :
                    #     print(f"\n ¨** Only a single profile inverted, taken the mean of STD as STD")
                    #     tmp_xarr_mean = tmp_xinv_arr.squeeze(axis=0)
                    #     tmp_xarr_std = local_std_coeff * tmp_xinv_arr_std.squeeze(axis=0)
                    # else:
                    tmp_xarr_mean = tmp_xinv_arr.mean(axis=0)
                    tmp_xarr_std = local_std_coeff * tmp_xinv_arr.std(axis=0,ddof=1)
                    tmp_xarr_meanstd = local_std_coeff * tmp_xinv_arr_std.mean(axis=0)  # mean of all STD (each inversion)
                    print('tmp_xarr_mean shape:',tmp_xarr_mean.shape, 'tmp_xarr_std shape:',tmp_xarr_std.shape, 'tmp_xarr_meanstd shape:',tmp_xarr_meanstd.shape)
                    ...
                    if mod_to_invert in obs_set:
                        # uniquement les HIST/Obs selectionnes dans l'inversion (dans la liste mlt_list_nbe)
                        HIST_obs_for_inv_array = obs_df.values[mlt_list_nbe,:].copy()
                    ...
                    tmp_y_err = local_std_coeff * tmp_yinv_arr.std(axis=0,ddof=1)
                    tmp_y_meanerr = local_std_coeff * tmp_yinv_arr_std.mean(axis=0) # mean of STD
